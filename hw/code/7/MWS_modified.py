...
def energy(list2,MWS_MIN,MWS_MAX):
...
def MaxWalkSat(mws1):
	...
	bestEnergy=best.eval()
        #print best.decisionList
	formatList=[]
	for i in range(trials):
		new_list=mws1()
		#print new_list
		for j in range(changes):
			#print new_list.decisionList	
			if (new_list.eval()<threshold):
				#print "if1"
				string1="With energy "+str(normalized(new_list.eval(),MWS_MIN,MWS_MAX))+" Best Solution now is "+str(new_list.decisionList)
				print string1
				return 0
			...
			if (new_list.eval()<best.eval()):
				#print "if2"
				best=mws1()
				best=copy.deepcopy(new_list)
				formatList.append('!')
			elif (new_list.eval()<best.eval()):
				formatList.append('+')	
				#print "random"
			elif (new_list.eval()==best.eval()):
                                formatList.append('.')
				...
				if (intr_list.eval()==new_list.eval()):
					formatList.append('.')
				else:					formatList.append('+')"""
			if (len(formatList)>=100):
				print i,"   ",	
				print "".join(formatList)
				formatList=[]
	string1="With energy "+str(normalized(best.eval(),MWS_MIN,MWS_MAX))+" Best Solution now is "+str(best.decisionList)
        print string1
